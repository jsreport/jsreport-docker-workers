...
module.exports = ({ reporter, containersManager, ip, stack, serversChecker, discriminatorPath }) => {
  ...
  async function allocateWorker ({ discriminator,
    req
  }) {
    ...
    if (currentTenantWorker && currentTenantWorker.ip !== ip) {
  ...
  return async (req, fn) => {
    ...
    try {
      ...
      await containersManager.recycle({ container, originalTenant: discriminator })
      throw e
    }
