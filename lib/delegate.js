      ...
      const response = await new Promise((resolve, reject) => {
        ...
        const postReq = http.request(requestOpts, (res) => {
          ...
          res.on('end', () => {
            ...
            let obj
            try {
              obj = serializator.parse(d)
            } catch (e) {
              reporter.logger.error(`Failed to parse response from worker: ${d}`)
              const reqErr = new Error(`Failed to process worker response, status: ${response.status}`)
        ...
        if (delegateTimeout != null) {
          const timeout = isRemote ? delegateTimeout * 2 : delegateTimeout
          postReq.setTimeout(timeout, () => {
            postReq.abort()
            const reqErr = new Error(`Container delegate request timeout after ${timeout}ms`)
            reject(reqErr)
          })
        }
