...
module.exports = (reporter, { onRequestFilter, onResponseFilter, onContainerError }) => {
  async function axiosPost (url, data, opts) {
    ...
    try {
      ...
      const payloadBuffer = reporter.serializer.serialize(dataToSend)

      const response = await axios.post(url, {
        payload: payloadBuffer.toString('hex')
      }, opts)

      responseDataBuf = Buffer.from(response.data.payload, 'hex')
      response.data = reporter.serializer.deserialize(responseDataBuf)
      response.data = await onResponseFilter(response.data)
