...
module.exports = ({
  ...
  const busyQueue = createBusyQueue({
    run: (item) => {
      logger.info(`Continuing queued work and trying to get a worker (discriminator: ${item.tenant})`)
  ...
  const waitForContainerRestart = async (container, tenant, logger) => {
    try {
      logger.info(`Waiting until docker container's restart finish ${container.id} (${container.url}) (discriminator: ${tenant})`)
      await container.restartPromise
      logger.info(`Docker container's restart was ok ${container.id} (${container.url}) (discriminator: ${tenant})`)
    } catch (e) {
  ...
  async function allocate ({req, tenant}) {
    ...
    if (container) {
      logger.info(`Reusing existing docker container ${container.id} (${container.url}) (discriminator: ${tenant})`)
    ...
    if (container.numberOfRequests > 0) {
      logger.info(`All docker containers are busy, queuing work and waiting for a worker to be available (discriminator: ${tenant})`)
    ...
    if (!originalTenant) {
      ...
      logger.info(`No need to restart unassigned docker container ${container.id} (discriminator: ${tenant})`)
      return container
    }

    logger.info(`Restarting and unregistering previous assigned docker container ${container.id} (discriminator: ${tenant})`)

    await recycle({
      container,
      originalTenant,
      onRestart: (c) => {
        logger.info(`Restoring information for new work after unregistering previous assigned docker container ${container.id} (discriminator: ${tenant})`)
