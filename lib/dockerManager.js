...
module.exports = (reporter, {
  ...
  containerLogDriver,
  containerLogOpt,
  containerTempVolumeSourcePrefix,
  containerTempVolumeTarget,
  subnet,
  network,
  ...
  if (!containerTempVolumeSourcePrefix) {
    containerTempVolumeSourcePrefix = path.join(reporter.options.tempDirectory, 'workers')
  }
  ...
  if (!path.isAbsolute(containerTempVolumeSourcePrefix)) {
    throw new Error(`"containerTempVolumeSourcePrefix" option needs to be an absolute path`)
  }

  if (!path.isAbsolute(containerTempVolumeTarget)) {
    throw new Error(`"containerTempVolumeTarget" options needs to be an absolute path`)
  }
  ...
  const containersManager = createContainersManager({
    ...
    logOpt: containerLogOpt,
    tempVolumeSourcePrefix: containerTempVolumeSourcePrefix,
    tempVolumeTarget: containerTempVolumeTarget,
    subnet,
    network,
    maxContainers,
