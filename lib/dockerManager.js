...
module.exports = (reporter, definition) => {
  ...
  async function getContainer ({
    ...
    if (currentTenantWorker && currentTenantWorker.ip != null && currentTenantWorker.ip !== ip && serversChecker.status(currentTenantWorker.ip)) {
      ...
      const remoteUrl = `http://${currentTenantWorker.ip}:${serverPort}/api/worker-docker-manager`

      reporter.logger.info(`Delegating request to external worker ${currentTenantWorker.ip} (${remoteUrl}), discriminator: ${discriminator}`, req)
    ...
    await reporter.documentStore.collection('tenantWorkers').update({
      stack,
      tenant: discriminator
    }, { $set: { ip, stack, tenant: discriminator, updateAt: new Date() } }, { upsert: true })
  ...
  async function executeScript (inputs, options, req) {
    ...
    if (worker.remote === true) {
      reporter.logger.debug(`Delegating script to external worker (${worker.url})`, req)
    } else {
      reporter.logger.debug(`Delegating script to container in local worker (${worker.url})`, req)
    }
    ...
    try {
      ...
      // TODO: make authenticated request when remote
      resp = await axiosPost(worker.url, requestInput)
    } catch (e) {
      if (!worker.remote) {
  ...
  async function delegateRecipe (worker, recipe, req, res, fromRemote) {
    ...
    if (worker.remote === true) {
      ...
      reporter.logger.debug(`Delegating recipe ${recipe} to container in local worker at ${url}`, req)
  ...
  async function axiosPost (url, opts) {
    try {
