...
module.exports = (reporter, {
  ...
  const delegate = createDelegate(reporter, {
    ...
    onRequestFilter: async (type, originalReq, reqData) => {
      const pipe = {
        type,
        req: originalReq,
        data: reqData
    ...
    onResponseFilter: async (type, originalReq, resData) => {
      const pipe = {
        type,
        req: originalReq,
        data: resData
  ...
  return {
    ...
    addContainerDelegateRequestFilterListener (name, fn) {
      containerDelegateRequestFilterListeners.add(name, async (opts) => {
        ...
        const { type, req, data } = opts

        const customData = await fn(type, req, data)
