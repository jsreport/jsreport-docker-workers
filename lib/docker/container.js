...
module.exports = class Container {
  constructor ({
    ...
    this.enableRequestTempFiles = enableRequestTempFiles
    ...
    if (container.tempVolumeSourcePrefix) {
      this.tempVolumeHostPathOriginal = path.join(container.tempVolumeSourcePrefix, 'jsreport', 'workers', this.id)
    } else {
      this.tempVolumeHostPathOriginal = path.join(this.tempDirectory, 'workers', this.id)
    }

    this.tempAutoCleanupDirectoryPathInHost = path.join(this.tempVolumeHostPathOriginal, 'autocleanup')
    this.tempVolumeHostPath = normalizeWindowsPathForDocker(this.tempVolumeHostPathOriginal)
  }

  async start () {
    try {
      // the volume mount needs to run on already existing path
      await cleanWorkerTemp(this.tempVolumeHostPathOriginal)
    ...
    try {
      ...
      if (this.enableRequestTempFiles === true || this.enableRequestTempFiles === 'true') {
        runCMD += ` --env workerEnableRequestTempFiles=true`
      }
